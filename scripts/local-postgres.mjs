...
if (
  ...
  throw new Error('Local PostgreSQL expects cbredb on localhost:5433 in Api/.env.');
...
if (command === 'stop') {
  ...
  if (!fs.existsSync(path.join(data, 'PG_VERSION'))) {
    ...
    try {
      fs.writeFileSync(passwordFile, decodeURIComponent(url.password), {
        mode: 0o600,
        flag: 'wx',
      });
      run('initdb', [
  ...
  for (let attempt = 0; attempt < 30; attempt++) {
    ...
    try {
      ...
      await candidate.end();
      if (occupied || attempt === 29) throw error;
      await new Promise((resolve) => setTimeout(resolve, 250));
    }
  }
  try {
    const directory = (await connection.query('SHOW data_directory')).rows[0]
      .data_directory;
    if (path.resolve(directory).toLowerCase() !== path.resolve(data).toLowerCase())
      throw new Error(
        'Port 5433 belongs to another PostgreSQL cluster; no changes made.',
      );
  ...
  throw new Error('Usage: npm run db:local -- [start|stop]');
}
